...
void decrypt()
{
    ...
    for (int i = 0; i < digraph.size(); i++)
    {
        ...
        if (x1 == x2)
        {
            digraph[i].first = grid[x1][(((y1 - 1) % 5) + 5) % 5];
            digraph[i].second = grid[x2][(((y2 - 1) % 5) + 5) % 5];
        }
        else if (y1 == y2)
        {
            digraph[i].first = grid[(((x1 - 1) % 5) + 5) % 5][y1];
            digraph[i].second = grid[(((x2 - 1) % 5) + 5) % 5][y2];
        }
        else
        {
...
{
    ofstream plaintext;
    plaintext.open("decrypted.txt");
    for (int i = 0; i < digraph.size(); i++)
    {
        plaintext << digraph[i].first << digraph[i].second;
    }
    plaintext.close();
}
